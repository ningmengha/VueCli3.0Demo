...
export default {
  ...
  methods: {
    localDate:function(_data){
        var data = new Date(_data);
        return  data && data.getFullYear()+'-'+(data.getMonth()<9?'0'+(data.getMonth()+1):data.getMonth()+1)+'-'+(data.getDate()<10?'0'+data.getDate():data.getDate())+ ' '+ (data.getHours()<10 ? '0'+data.getHours():data.getHours()) + ':'+ (data.getMinutes() < 10?'0'+ data.getMinutes():data.getMinutes()) +':'+ (data.getSeconds() < 10 ? "0"+data.getSeconds():data.getSeconds());
      },
    getProfile() {
      // 获取表格数据
      this.$axios("/api/profiles/list").then(res => {
        // this.tableData = res.data;
        this.allTableData = res.data.list;
        this.filterTableData = res.data.list;
        this.allTableData.forEach((item) => {
          item.createTime = this.localDate(item.date)
        })
        // 设置分页数据
        this.setPaginations();
      });
